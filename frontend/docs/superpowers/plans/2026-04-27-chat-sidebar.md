...
**Goal:** Replace the floating chat popup in the React frontend with a persistent right-side panel that compresses the main content and preserves chat history across page navigation.

**Architecture:** A `ChatContext` provider above the router holds `isOpen`, `toggle`, `messages`, and `setMessages`. `App.tsx` becomes a flex row — `flex-1 min-w-0 overflow-y-auto` for the page content and a `ChatSidebar` that animates its width from `w-0` to `w-[360px]`. The `AppLayout` header gains a toggle button reading from context.

**Tech Stack:** React 19, TypeScript, Tailwind CSS v4, Vite, React Router v7
...
|---|---|---|
| `src/context/ChatContext.tsx` | Create | Shared open state + message history |
| `src/components/chatbot/ChatSidebar.tsx` | Create | Full sidebar UI, sendMessage logic, card sub-components |
| `src/App.tsx` | Modify | Flex-row root layout, wrap in ChatProvider, remove BibleChatWidget |
| `src/components/layout/AppLayout.tsx` | Modify | Add chat toggle button to header |
| `src/components/chatbot/BibleChatWidget.tsx` | Unchanged | UMD build only — do not touch |

---

### Task 1: Create ChatContext
...
git commit -m "feat: add ChatContext for persistent sidebar state"
```

---

### Task 2: Create ChatSidebar component

**Files:**
- Create: `src/components/chatbot/ChatSidebar.tsx`
...
```bash
git add src/components/chatbot/ChatSidebar.tsx
...
```bash
git add src/App.tsx
git commit -m "feat: restructure App.tsx with flex-row layout and ChatProvider"
```

---

### Task 4: Add toggle button to AppLayout header
...
git commit -m "feat: add chat toggle button to AppLayout header"
```
...
Ensure the FastAPI backend (`start-chatbot.sh`) and Flask API (`python myproject.py`) are running, then:

```bash
cd /Volumes/HomeX/Chris/Documents/Bible-Gematria-Interlinear-Explorer/frontend
npm run dev
```
...
- Click the **Chat** button in the header → sidebar slides in from the right, main content compresses left
- Click **Chat** again → sidebar slides closed, content expands back
- Click the **×** inside the sidebar → same close behaviour

- [ ] **Step 3: Verify chat history persists across navigation**

- Open the sidebar, send a message (e.g. `John 3:16`) and wait for the response
- Click **Strong's** in the nav, then click **Explorer** — the chat messages should still be there

- [ ] **Step 4: Verify a verse lookup response renders correctly**

- Type `John 3:16` and send → response should show a `VerseCard` with translation text and version buttons (eng-NIV, eng-KJV, etc.)

- [ ] **Step 5: Verify an AI response renders correctly**

- Type `What is the significance of the word logos in John 1:1?` and send → response should show a plain text AI reply after a few seconds

- [ ] **Step 6: Commit if any polish fixes were needed**

```bash
git add -p
git commit -m "fix: chat sidebar polish"
```
