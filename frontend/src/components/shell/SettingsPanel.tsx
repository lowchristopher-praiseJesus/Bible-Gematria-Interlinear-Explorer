              ...
              <section className="flex flex-col gap-2">
                ...
                <label
                  className={cn(
                    ...
                    openaiApiKey ? 'text-[var(--color-text-primary)]' : 'cursor-not-allowed text-[var(--color-text-secondary)] opacity-60',
                  )}
                >
                  <input
                    type="checkbox"
                    checked={useOpenAiForResponses}
                    disabled={!openaiApiKey}
                    onChange={(e) => setUseOpenAiForResponses(e.target.checked)}
                    className="h-4 w-4 shrink-0 accent-[var(--color-theme-accent)]"
                  />
                  Use OpenAI (gpt-5.4-mini) to generate voice responses
                </label>
                <p className="text-xs leading-relaxed text-[var(--color-text-secondary)]">
                  When enabled, the answer spoken back during a voice session is generated by
                  OpenAI&apos;s gpt-5.4-mini using your key above, instead of this app&apos;s
                  default model. Typed chat is unaffected.
                </p>
              </section>
