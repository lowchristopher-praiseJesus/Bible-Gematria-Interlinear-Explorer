...
<div class="m_3 clear"></div>
	...
	<script src="/static/chatbot-widget.umd.js"></script>
	<script>
		// Initialize chat widget when DOM is ready
		document.addEventListener('DOMContentLoaded', function() {
			const container = document.getElementById('chatbot-widget-root');
			if (container && window.BibleChatWidget && React && ReactDOM) {
				const root = ReactDOM.createRoot(container);
				root.render(
					React.createElement(window.BibleChatWidget.BibleChatWidget, {
						apiUrl: '/api/bible-chat',
						position: 'bottom-right',
						theme: 'light',
						title: 'Bible Study Chat',
						welcomeMessage: 'Ask me about any verse, commentary, or Greek/Hebrew word.'
					})
				);
			} else {
				console.error('Chat widget failed to load: missing dependencies');
			}
		});
	</script>
</body>
...
@cache.memoize(timeout=536112000)
def explorer_api_data(versenum, previous_versenum, next_versenum):
	db = dataset.connect(DB_PATH)
	Complete = db['Complete']
	Strongs = db['Strongs_']
	...
	language = 'Hebrew' if versenum < 23146 else 'Greek'

	alert = None
	if versenum in (6418, 6419, 12489):
		alert = 'This verse does not occur in the Leningrad Codex'
	elif versenum == 25688:
		alert = 'This verse does not appear in the Stephanus 1550'

	lc_files = []
	if language == 'Hebrew' and LC:
		if ',' in LC:
			lc_files = [LC.split(',', 1)[1], LC.split(',', 1)[0]]
		else:
			lc_files = [LC]
	...
	stephanus_text = None
	if language == 'Hebrew':
		original_text = Original.replace('<pe ', '<span class="item_container"><pe ').replace('</pe>', '</pe></span>')
		original_text = original_text.replace('<samekh ', '<span class="item_container"><samekh ').replace('</samekh>', '</samekh></span>')
	else:
		stephanus_text = Greek_Letters_Rules(Stephanus) if Stephanus else None

	has_qere = '<q' in row['Original_Words']

	all_sns = []
	for number in KJV_SN + SNs_ + Original_Words_SN:
		if number != 'NONE' and number not in all_sns:
			all_sns.append(number)

	strongs_defs = {}
	for result in Strongs.find(StrongsNumber=all_sns):
		if result['Root'] is not None:
			strongs_defs[result['StrongsNumber']] = strongs_row_to_dict(result)

	kjv_words = []
	if language == 'Hebrew':
		for kjv_t, kjv_sn, rt, rt2, root, rv in zip(KJV_Text, KJV_SN, Root_Translit, Root_Translit2, Root, Root_val):
			kjv_words.append({
				'kjvText': kjv_t.replace('<s ', '<st ').replace('</s>', '</st>'),
				'strongsNumber': kjv_sn,
				'root': root,
				'rootTranslit': Hebrew_Translit_Rules(rt),
				'rootTranslit2': rt2,
				'rootVal': rv,
			})
	else:
		for kjv_t, kjv_sn, rt, root, rv in zip(KJV_Text, KJV_SN, Root_Translit, Root, Root_val):
			kjv_words.append({
				'kjvText': kjv_t.replace('<s ', '<st ').replace('</s>', '</st>'),
				'strongsNumber': kjv_sn,
				'root': root,
				'rootTranslit': Greek_Translit_Rules(rt),
				'rootTranslit2': None,
				'rootVal': rv,
			})
	...
	original_words = []
	if language == 'Hebrew':
		for ow_sn, ow_tl, ow_tl2, ow, ow_val in zip(Original_Words_SN, Original_Words_Translit, Original_Words_Translit2, Original_Words, Original_Words_values):
			val = None
			if ow_val != 'NONE' and ow_val != '':
				try: val = int(ow_val)
				except: pass

			asn = ow.split('asn="', 1)[1].split('"', 1)[0] if 'asn=' in ow else None
			gsn = None
			gsn_parts = None
			if 'gsn=' in ow:
				gsn = ow.split('gsn="', 1)[1].split('"', 1)[0]
				gsn_parts = ow.split('parts="', 1)[1].split('"', 1)[0]

			is_qere = '<q' in ow
			word_html = ow
			if is_qere:
				word_html = word_html.replace('<q', '<qere').replace('</q>', '</qere>')

			translit = None
			if ow_tl == 'NONE':
				word_html = word_html.replace('<reversednun ', '<span class="item_container"><reversednun ').replace('</reversednun>', '</reversednun></span>')
			else:
				translit = Hebrew_Translit_Rules(ow_tl)
				word_html = word_html.replace('</esv><esv', '</esv>&nbsp;<esv').replace('<esv', '<span class="item_container"><esv').replace('</esv>', '</esv></span>')
		...
		for ow_sn, ow_tl, ow, ow_val in zip(Original_Words_SN, Original_Words_Translit, Original_Words, Original_Words_values):
			val = None
			if ow_val != 'NONE' and ow_val != '':
				try: val = int(ow_val)
				except: pass
			...
			gsn_parts = None
			if 'gsn=' in ow:
				gsn_tag = ow.split('gsn="', 1)[1].split('"', 1)[0]
				gsn = gsn_tag.split(',', 1)[0]
				gsn_parts = gsn_tag.split(',', 1)[1]
			add_strongs = ow.split('AddStrongs="', 1)[1].split('"', 1)[0] if 'AddStrongs=' in ow else None

			word_html = ow.replace('<w', '<span class="item_container"><gw').replace('</w>', '</gw></span>')
	...
	return {
		'verse': {
			...
			'total': total,
			'text1769': text_1769,
			'textAV1611': text_AV_1611,
			'language': language,
			'originalText': original_text,
			'stephanusText': stephanus_text,
			'stephanusTotal': Stephanus_total,
			'lcFiles': lc_files,
			'hasQere': has_qere,
			'code': Code,
			'alert': alert,
...
@cache.memoize(timeout=536112000)
def apoc_api_data(reference, apoc_previous, apoc_next):
	db = dataset.connect(DB_PATH)
	text = ''
	for result in db.query('SELECT * FROM APOC WHERE ref LIKE :r LIMIT 1', r='%' + reference + '%'):
		text = result['text']
	return {
		'isApocrypha': True,
		'reference': reference,
		'text': text,
		'previous': apoc_previous,
		'next': apoc_next,
	}


@cache.memoize(timeout=536112000)
def strongs_api_data(strongsnumber):
	db = dataset.connect(DB_PATH)
	Strongs = db['Strongs_']

	rows = []
	for result in db.query('SELECT * FROM Complete WHERE SNs_ LIKE :sn', sn='%~' + strongsnumber + '~%'):
		rows.append(result)

	definition_row = Strongs.find_one(StrongsNumber=strongsnumber)
	definition = None
	if definition_row is not None and definition_row['Root'] is not None:
		definition = strongs_row_to_dict(definition_row)

	if len(rows) == 0:
		return {'definition': definition, 'verses': [], 'resultSummary': 'No results'}

	num_verses = len(rows)
	num_books = len(list(set([r['book'] for r in rows])))
	summary = str(num_verses) + ' verse' + ('s' if num_verses != 1 else '') + ' found in ' + str(num_books) + ' book' + ('s' if num_books != 1 else '')
	...
	for item in rows:
		if item['book'] not in books_list:
			books_list.append(item['book'])
			bcv_list.append([])
		bcv_list[len(books_list) - 1].append({'id': item['id'], 'bnum': item['bnum'], 'cnum': item['cnum'], 'vnum': item['vnum'], 'ref': item['ref']})

	verses = [{'book': b, 'refs': refs} for b, refs in zip(books_list, bcv_list)]
	return {'definition': definition, 'verses': verses, 'resultSummary': summary}


@cache.memoize(timeout=536112000)
def gematria_api_data(value):
	db = dataset.connect(DB_PATH)
	Strongs = db['Strongs_']

	rows = []
	rows2 = []
	for result in db.query('SELECT * FROM Complete WHERE Original_Words_values LIKE :otv LIMIT ' + str(ROW_RESULT_LIMIT), otv='%~' + str(value) + '~%'):
		rows.append(result)
	for result in db.query('SELECT * FROM Complete WHERE total = :value', value=value):
		rows2.append(result)
	...
	for row in rows:
		...
		for sn, ow, ov in zip(sn_list, ow_list, val_list):
			...
			try:
				if int(ov) == value:
					word_results.append({'id': row['id'], 'ref': row['ref'], 'bnum': row['bnum'], 'cnum': row['cnum'], 'vnum': row['vnum'], 'strongsNumber': sn, 'wordHtml': ow, 'language': lang})
	...
	for row in rows2:
		verse_results.append({'id': row['id'], 'ref': row['ref'], 'bnum': row['bnum'], 'cnum': row['cnum'], 'vnum': row['vnum'], 'total': row['total'], 'text1769': row['text_1769']})
	...
	for result in Strongs.find(StrongsNumber=all_sns):
		if result['Root'] is not None:
			strongs_defs[result['StrongsNumber']] = strongs_row_to_dict(result)
			sn_data[result['StrongsNumber']] = {
				'transliteration1': result['Transliteration1'],
	...
	for w in word_results:
		...
		if sn in sn_data:
			if w['language'] == 'H':
				w['snTranslit'] = Hebrew_Translit_Rules(sn_data[sn]['transliteration1'])
			else:
				w['snTranslit'] = Greek_Translit_Rules(sn_data[sn]['transliteration'])
			w['snMeaning'] = sn_data[sn]['meaning']
		else:
			w['snTranslit'] = ''
			w['snMeaning'] = ''

	if H_count == 0 and G_count == 0:
		summary_words = 'No results'
	else:
		parts = []
		if H_count > 0: parts.append(str(H_count) + ' Hebrew word' + ('s' if H_count != 1 else ''))
		if G_count > 0: parts.append(str(G_count) + ' Greek word' + ('s' if G_count != 1 else ''))
		summary_words = ', '.join(parts)
		if len(rows) == ROW_RESULT_LIMIT: summary_words += ' (' + str(ROW_RESULT_LIMIT) + ' row result limit reached)'

	v_count = len(verse_results)
	b_count = len(set(v['bnum'] for v in verse_results))
	summary_verses = 'No results' if v_count == 0 else str(v_count) + ' verse' + ('s' if v_count != 1 else '') + ' found in ' + str(b_count) + ' book' + ('s' if b_count != 1 else '')

	return {
		'wordResults': word_results,
		'verseResults': verse_results,
		'strongsDefinitions': strongs_defs,
		'resultSummaryWords': summary_words,
		'resultSummaryVerses': summary_verses,
	}


@cache.memoize(timeout=536112000)
def english_api_data(words):
	db = dataset.connect(DB_PATH)
	rows = []
	for result in db.query("SELECT * FROM Complete WHERE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(text_1769, '</i>', ''), '<i>', ''), '</divine>', ''), '<divine>', ''), '</inscription>', ''), '<inscription>', ''), '</psalmheader>', ''), '<psalmheader>', ''), '</headingletter>', ''), '<headingletter>', ''), '</colophon>', ''), '<colophon>', '') LIKE :words LIMIT " + str(ROW_RESULT_LIMIT), words='%' + words + '%'):
		rows.append(result)

	if len(rows) == 0:
		return {'results': [], 'resultSummary': 'No results', 'searchTerm': words}

	num_verses = len(rows)
	num_books = len(list(set([r['book'] for r in rows])))
	summary = str(num_verses) + ' verse' + ('s' if num_verses != 1 else '') + ' found in ' + str(num_books) + ' book' + ('s' if num_books != 1 else '')
	if num_verses == ROW_RESULT_LIMIT: summary += ' (' + str(ROW_RESULT_LIMIT) + ' row result limit reached)'

	results = []
	for row in rows:
		plain = remove_tags(row['text_1769'], '<', '>')
		positions = find_match_positions(plain, words)
		results.append({
			'id': row['id'],
			'ref': row['ref'],
			'bnum': row['bnum'],
	...
	return {'results': results, 'resultSummary': summary, 'searchTerm': words}
...
def api_explorer():
	...
	if reference != None:
		reference = reference.lower().strip()
		if 'psalms' in reference: reference = reference.replace('psalms', 'psalm')
					...
					if ' ' in reference:
						...
						if the_book in THE_BOOKS_LOWER:
							...
							if first_n.isdigit():
								first_n = int(first_n)
								check_c = THE_REFS[the_book][0]
								the_reference = the_book + ' ' + str(min(first_n, check_c) if first_n > 0 else 1) + ':1'
							else:
								the_reference = the_book + ' 1:1'
			else:
				if ' ' in reference:
							...
							if first_n > 0:
								...
								if first_n <= check_c:
									...
									if second_n.isdigit():
										second_n = int(second_n)
										check_v = THE_REFS[the_book][1][first_n]
										the_reference += ':' + str(min(second_n, check_v) if second_n > 0 else 1)
									else:
										the_reference += ':1'
								else:
									the_reference = the_book + ' ' + str(check_c) + ':1'
		...
		if the_reference != '':
			...
			if the_book_index < 66:
				previous_verse_count = sum(sum(THE_REFS[b][1].values()) for b in THE_BOOKS[:the_book_index])
				previous_verse_count += v_num
				if c_num > 1:
					for i in range(1, c_num): previous_verse_count += THE_REFS[the_book][1][i]
				versenum = previous_verse_count
				if versenum == 1: previous_versenum, next_versenum = 31102, 2
				elif versenum == 31102: previous_versenum, next_versenum = 31101, 1
				else: previous_versenum, next_versenum = versenum - 1, versenum + 1

			if the_reference == 'Genesis 1:1': the_previous, the_next = 'Revelation 22:21', 'Genesis 1:2'
			elif the_reference == 'Revelation 22:21': the_previous, the_next = 'Revelation 22:20', 'Genesis 1:1'
			elif the_reference == '1 Esdras 1:1': the_previous, the_next = '', '1 Esdras 1:2'
			elif the_reference == '2 Maccabees 15:39': the_previous, the_next = '2 Maccabees 15:38', ''
			else:
				if v_num < max_verse: the_next = the_book + ' ' + str(c_num) + ':' + str(v_num + 1)
				elif c_num + 1 <= max_chapter: the_next = the_book + ' ' + str(c_num + 1) + ':1'
				else: the_next = THE_BOOKS[the_book_index + 1] + ' 1:1'

				if c_num == 1:
					if v_num > 1: the_previous = the_book + ' ' + str(c_num) + ':' + str(v_num - 1)
					else:
						prev_book = THE_BOOKS[the_book_index - 1]
						max_ch_prev = THE_REFS[prev_book][0]
						the_previous = prev_book + ' ' + str(max_ch_prev) + ':' + str(THE_REFS[prev_book][1][max_ch_prev])
				else:
					if v_num > 1: the_previous = the_book + ' ' + str(c_num) + ':' + str(v_num - 1)
					else: the_previous = the_book + ' ' + str(c_num - 1) + ':' + str(THE_REFS[the_book][1][c_num - 1])

			if the_book_index > 65:
				return jsonify(apoc_api_data(the_reference, the_previous, the_next))
		...
		versenum = versenum.strip()
		if versenum == '0' or all(c == '0' for c in versenum): versenum = '0'
		if not versenum.isdigit() or versenum == '0': versenum = 1
		versenum = int(versenum)
		if versenum > 31102: versenum = 31102
		if versenum == 1: previous_versenum, next_versenum = 31102, 2
		elif versenum == 31102: previous_versenum, next_versenum = 31101, 1
		else: previous_versenum, next_versenum = versenum - 1, versenum + 1

	elif book != None and chapter != None and verse != None:
		book = book.strip(); chapter = chapter.strip(); verse = verse.strip()
		if book == '0' or all(c == '0' for c in book): book = '0'
		if chapter == '0' or all(c == '0' for c in chapter): chapter = '0'
		if verse == '0' or all(c == '0' for c in verse): verse = '0'
		try:
			if book.isdigit() and chapter.isdigit() and verse.isdigit() and book != '0' and chapter != '0' and verse != '0':
				book = int(book); chapter = int(chapter); verse = int(verse)
				if book <= 66 and chapter <= THE_REFS[THE_BOOKS[book - 1]][0] and verse <= THE_REFS[THE_BOOKS[book - 1]][1][chapter]:
					previous_verse_count = sum(sum(THE_REFS[b][1].values()) for b in THE_BOOKS[:book - 1])
					previous_verse_count += verse
					if chapter > 1:
						for i in range(1, chapter): previous_verse_count += THE_REFS[THE_BOOKS[book - 1]][1][i]
					versenum = previous_verse_count
					if versenum == 1: previous_versenum, next_versenum = 31102, 2
					elif versenum == 31102: previous_versenum, next_versenum = 31101, 1
					else: previous_versenum, next_versenum = versenum - 1, versenum + 1
			...
			versenum = 1; previous_versenum = 31102; next_versenum = 2
	else:
		versenum = 1; previous_versenum = 31102; next_versenum = 2

	return jsonify(explorer_api_data(versenum, previous_versenum, next_versenum))


@app.route('/api/strongs', methods=['GET'])
def api_strongs():
	strongsnumber = request.args.get('strongsnumber')
	if strongsnumber == None: strongsnumber = 'H2442'
	strongsnumber = strongsnumber.strip()
	...
	strongsnumber = strongsnumber[:1].upper() + strongsnumber[1:].lstrip('0')
	if strongsnumber == 'H' or strongsnumber == 'G': strongsnumber = 'H2442'
	return jsonify(strongs_api_data(strongsnumber))


@app.route('/api/gematria', methods=['GET'])
def api_gematria():
	value = request.args.get('value')
	if value == None: value = '1489'
	else: value = value.strip()
	if not value.isdigit(): value = 1489
	if int(value) < 1 or int(value) > 40000: value = 1489
	value = int(value)
	return jsonify(gematria_api_data(value))
...
def api_english():
	words = request.args.get('words')
	if words == None: words = 'hammer'
	if not any(c.isalpha() for c in words): words = 'hammer'
	if len(words) > 66 or len(words) == 1: words = 'hammer'
	return jsonify(english_api_data(words))


@app.route('/api/books', methods=['GET'])
def api_books():
	return jsonify({'books': THE_BOOKS, 'booksLower': THE_BOOKS_LOWER, 'refs': THE_REFS})


@app.route('/api/apoc', methods=['GET'])
def api_apoc():
	reference = request.args.get('reference', '').strip()
	if not reference:
		return jsonify({'error': 'reference parameter required'}), 400
	return jsonify(apoc_api_data(reference, '', ''))


@app.route('/LC_/<path:filename>')
def lc_images(filename):
